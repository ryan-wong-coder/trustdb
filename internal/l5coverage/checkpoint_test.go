...
func TestAdvanceIsMonotonic(t *testing.T) {
	...
	first, changed, err := Advance(model.L5CoverageCheckpoint{}, false, cryptosuite.INTLV1, key, 8, 100)
	if err != nil || !changed || first.CoveredTreeSize != 8 || first.Revision != 1 {
		t.Fatalf("Advance first = %+v changed=%v err=%v", first, changed, err)
	}
	unchanged, changed, err := Advance(first, true, cryptosuite.INTLV1, key, 5, 200)
	if err != nil || changed || unchanged != first {
		t.Fatalf("Advance lower = %+v changed=%v err=%v", unchanged, changed, err)
	}
	advanced, changed, err := Advance(first, true, cryptosuite.INTLV1, key, 12, 300)
...
func TestAdvanceRejectsMismatchedKey(t *testing.T) {
	key := model.STHAnchorScheduleKey{NodeID: "node-1", LogID: "log-1", SinkName: "file"}
	current, _, err := Advance(model.L5CoverageCheckpoint{}, false, cryptosuite.INTLV1, key, 8, 100)
	if err != nil {
		t.Fatalf("Advance first: %v", err)
	}
	other := key
	other.SinkName = "ots"
	if _, _, err := Advance(current, true, cryptosuite.INTLV1, other, 12, 200); trusterr.CodeOf(err) != trusterr.CodeDataLoss {
