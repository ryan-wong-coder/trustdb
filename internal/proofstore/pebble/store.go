...
const (
	...
	prefixAnchorSchedule = "anchor/schedule/v2/"
...
func OpenWithOptions(path string, opts Options) (*Store, error) {
	...
	if strings.TrimSpace(opts.NodeID) == "" || strings.TrimSpace(opts.LogID) == "" || strings.TrimSpace(opts.NamespaceID) == "" {
		return nil, trusterr.New(trusterr.CodeInvalidArgument, "pebble proofstore node_id, log_id, and namespace_id are required")
	}
...
func ensureStorageSchema(db *pdb.DB, expected cryptosuite.ID, nodeID, logID, namespaceID string) (proofstoremeta.Marker, error) {
	storageInitMu.Lock()
...
func (s *Store) rebuildManifestIdempotency(ctx context.Context, manifest model.BatchManifest) error {
	...
	if err != nil {
		return trusterr.Wrap(trusterr.CodeFailedPrecondition, "create idempotency crypto provider", err)
	}
...
func (s *Store) writeIdempotencyDecisionPage(decisions []model.IdempotencyDecision) error {
	...
	for i := range decisions {
		if err := s.requireSuite(decisions[i]); err != nil {
			return trusterr.Wrap(trusterr.CodeDataLoss, fmt.Sprintf("rebuilt idempotency decision %d", i), err)
		}
		storageKey, err := idempotency.StorageKeyForSuite(s.CryptoSuite(), decisions[i].Identity)
		if err != nil {
			return trusterr.Wrap(trusterr.CodeDataLoss, "derive rebuilt idempotency storage key", err)
		}
		...
		if err != nil {
			return trusterr.Wrap(trusterr.CodeDataLoss, "derive rebuilt idempotency decision key", err)
		}
		found, err := s.readCBOR(decisionKey, &existing)
		if err != nil {
		...
		if err := stageSet(batch, decisionKey, data); err != nil {
	...
	if opts.Level == 0 {
		...
		for i := range leaves {
			nodes[i] = model.BatchTreeNode{
				...
				CryptoSuite:    leaves[i].CryptoSuite,
...
func (s *Store) PublishCommittedBatch(ctx context.Context, manifest model.BatchManifest, bundles []model.ProofBundle) ([]model.IdempotencyDecision, error) {
	...
	for i := range bundles {
		if err := s.requireSuite(bundles[i]); err != nil {
			return nil, trusterr.Wrap(trusterr.CodeInvalidArgument, fmt.Sprintf("committed bundle %d", i), err)
		}
	}
	provider, err := trustcrypto.ProviderForSuite(s.CryptoSuite())
	if err != nil {
	...
	for i := range decisions {
		...
		if err := idempotency.ValidateDecisionForSuite(s.CryptoSuite(), decisions[i].Identity, decisions[i]); err != nil {
			return nil, trusterr.Wrap(trusterr.CodeInvalidArgument, "validate idempotency decision", err)
		}
		storageKey, err := idempotency.StorageKeyForSuite(s.CryptoSuite(), decisions[i].Identity)
		if err != nil {
			return nil, trusterr.Wrap(trusterr.CodeInvalidArgument, "derive idempotency storage key", err)
		}
