...
- Proofstore：支持 file、Pebble 与实验性的 TiKV 后端；TiKV 可作为共享 proofstore，让多个计算节点共享持久证明数据，实现存算分离。
...
| `metastore` / `metastore_path` | Proofstore 后端：`file`、`pebble` 或实验性的 `tikv`；TiKV 模式下 `metastore_path` 可提供逗号分隔的 PD endpoint，连接共享存储集群。 |
