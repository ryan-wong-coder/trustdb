...
当前 gRPC service 覆盖桌面客户端和 SDK 已使用的核心路径：health、submit claim、双向 claim streaming、record list/detail、proof bundle、roots/latest root、STH、global proof、anchor state 和 metrics。同时注册标准 `grpc.health.v1.Health` 服务，便于基础设施探活。
...
- 构造并签名文件 claim；
- 构造并提交结构化日志 claim，支持单条、有界批量和带背压的流式提交；
- 提交 signed claim 到 `POST /v1/claims`，通过 `POST /v1/claims/batch` 提交 HTTP 原生批量，或通过 gRPC 双向流提交；
