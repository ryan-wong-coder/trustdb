...
```http
GET /v2/records/{record_id}/status
```

批量查询（最多 1000 条）：

```http
POST /v2/records/status:batchGet
...
```http
GET /v2/status-subscriptions/{subscription_id}/events
...
```http
GET /v2/status-subscriptions/{subscription_id}/statuses
```

删除：

```http
DELETE /v2/status-subscriptions/{subscription_id}
```
