...
内置 `passphrase-dev-v1` 只用于开发、离线演示和互操作，使用 `PBKDF2-HMAC-SM3`：16-byte 随机 salt、默认 200000 次、固定允许 100000–2000000 次、16-byte KEK。passphrase 只从 `TRUSTDB_DEV_KEY_PASSPHRASE` 读取，不提供普通命令行明文 flag。该能力不能表述为 HSM、SDF、KMS 托管，不能推出产品认证、密评结论或生产私钥已进入合规硬件边界。
...
因此修改 descriptor/envelope metadata、nonce、ciphertext、tag、KDF salt/iterations、provider metadata、版本或算法都会在 signer 返回前失败。未知字段、重复 key、CBOR tag、indefinite value、trailing bytes、非 canonical bytes、超限 payload 和 future version 均拒绝；不存在 plaintext、旧格式或其他 provider fallback。
...
`key rewrap` 先认证旧 wrapped DEK 和私钥 ciphertext，再只替换 wrapped DEK。content nonce/ciphertext、私钥、公钥、KeyID、Registry V2 事件和历史验签引用不变。写入使用同目录 owner-only 临时文件、file fsync、atomic replace 和 directory fsync；不创建 plaintext 或 `.bak` 副本。

## 4. 密钥生成与兼容

`trustdb key generate` 的新默认值是 `sm4-envelope-v1`，缺少标准开发 passphrase 环境变量时 fail closed。原 `plaintext-dev-v1` 仅保留为显式 `--protection plaintext-dev-v1` 的开发兼容入口，不自动选择、不从 envelope 失败降级，也不得在生产部署使用。
...
- atomic no-replace/replace、无 secret backup artifact；
- CLI generate/resolve/rewrap 和 INTL/SM2 lifecycle resolver；
- `go test`、race、Linux/Windows build gate 与 secret-safe log/fixture review。
