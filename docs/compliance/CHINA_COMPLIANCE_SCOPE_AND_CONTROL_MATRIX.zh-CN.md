...
| `CRY-03` | `STD-SM2` `STD-SM2-USE`：SM2 签名互操作 | [`ADR-0007`](ADR-0007-CANONICAL-SM2-SM3-SIGNATURES.zh-CN.md) 已实现固定 user ID/ZA、canonical DER、65-byte SM2 公钥、suite/domain framing、provider-neutral signer/verifier、OpenSSL 3 oracle、并发和 fuzz 负向测试。生产 suite 启用、GmSSL/Tongsuo、SDK/CLI/Desktop 与硬件 provider 仍待 #452–#457，`Partial` | 管理身份参数并禁止 SDK 自设默认值 | Security & Cryptography | SM2 官方向量、OpenSSL oracle、strict DER/key/suite 负向测试、跨 SDK/CLI/Desktop 测试 | `G2` |
