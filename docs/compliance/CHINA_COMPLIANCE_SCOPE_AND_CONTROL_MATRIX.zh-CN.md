...
| `CRY-05` | `STD-MODULE` `RULE-CII-CRYPT`：生产私钥处于受控密码模块 | 核心已使用 provider-neutral Signer、不可导出 KeyHandle、公钥描述和 capability gate；[`ADR-0008`](ADR-0008-VERSIONED-KEY-DESCRIPTORS.zh-CN.md) 已实现 canonical software/remote/PKCS#11/SDF descriptor、单 provider 解析、capability/public-key exact match、secret-safe diagnostics 和无 fallback。真实 PKCS#11 与 SDF adapter/sidecar 待 #452/#453，`Partial` | 采购适用且在触发时经检测认证合格的设备/服务，并执行密钥仪式 | Security & Cryptography + Certification Applicant | ADR-0004/0008、descriptor/provider contract tests、产品证书、密钥仪式 | `G3` `G7` |
| `CRY-06` | `STD-39786`：完整密钥生命周期 | v1 descriptor 已记录 KeyID、suite、算法、provider、公钥、证书链和 provider reference；[`ADR-0009`](ADR-0009-SUITE-AWARE-KEY-REGISTRY-V2.zh-CN.md) 的 Registry V2 已实现不可变 suite manifest、canonical 签名/hash 链、SM2/INTL 注册、幂等导入、原子轮换、时点查询、撤销、compromise、证书有效期约束和崩溃尾帧恢复；[`ADR-0010`](ADR-0010-AUTHENTICATED-SM4-SOFTWARE-KEY-ENVELOPES.zh-CN.md) 已补齐软件私钥 envelope 与 KEK rewrap。生产 provider backup/destroy 与真实硬件适配仍待 #452–#453，`Partial` | 审批生成、分发、启用、备份、恢复、销毁 | Security & Cryptography | ADR-0008/0009/0010、密钥台账、事件链、轮换与撤销验证 | `G3` |
