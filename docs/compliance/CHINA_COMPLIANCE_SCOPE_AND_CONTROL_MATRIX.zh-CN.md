...
| ID | 依据与控制目标 | TrustDB 产品责任 / 当前状态 | 部署方责任 | Owner | 证据 | Gate |
| --- | --- | --- | --- | --- | --- | --- |
| `CRY-01` | `STD-39786` `STD-43207`：密码套件明确且不可静默切换 | [`ADR-0001`](ADR-0001-CRYPTOGRAPHIC-SUITES.zh-CN.md)、[`ADR-0002`](ADR-0002-CRYPTO-AGILITY-FORMATS.zh-CN.md)、[`ADR-0003`](ADR-0003-SM-CRYPTO-DEPENDENCIES-AND-VECTORS.zh-CN.md)、[`ADR-0004`](ADR-0004-PROVIDER-NEUTRAL-CRYPTO-CONTRACTS.zh-CN.md)、[`ADR-0005`](ADR-0005-IMMUTABLE-PROOFSTORE-SUITE-MARKERS.zh-CN.md)、`internal/cryptosuite`、`internal/formatregistry` 与 file/Pebble/TiKV marker contract 已固定 suite、格式代际、provider 和持久 namespace 边界；非空缺失、未知、损坏、配置 mismatch、backup restore 与 metastore migration 均 fail closed。V2 业务对象传播仍待 #454，`Partial` | 选择经批准 Profile，不在同一日志中切换 | Security & Cryptography | ADR、registry/provider/marker contract tests、schema、混用拒绝测试 | `G1` |
| `CRY-02` | `STD-SM3`：SM3 覆盖内容和树语义 | [`ADR-0006`](ADR-0006-SM3-AND-RFC6962-MERKLE-PROFILES.zh-CN.md) 已实现 suite-aware SM3 byte/streaming/fixed-width hash、RFC6962-SM3、Batch/Global Log/proof core 绑定、suite-specific idempotency key 和跨 suite fail-closed；4096 叶全证明、零分配验证与 OpenSSL oracle 已通过。`CN_SM_V1` 生产启用仍待 SM2 与 V2 全链路 #448/#454，`Partial` | 固定参数和实现版本 | Security & Cryptography | provider contract、官方/交叉向量、Merkle golden vectors、property/fuzz/benchmark | `G2` |
| `CRY-03` | `STD-SM2` `STD-SM2-USE`：SM2 签名互操作 | [`ADR-0007`](ADR-0007-CANONICAL-SM2-SM3-SIGNATURES.zh-CN.md) 已实现固定 user ID/ZA、canonical DER、65-byte SM2 公钥、suite/domain framing、provider-neutral signer/verifier、OpenSSL 3 oracle、并发和 fuzz 负向测试；CLI 已可生成开发 SM2 descriptor 并执行 Registry V2 生命周期。隔离的 PKCS#11 sidecar 已要求显式厂商 SM2 mechanism/参数/输出格式、启动期 `CKF_SIGN` gate 和 Host 本地反向验签；生产 suite 启用、目标硬件互认证、GmSSL/Tongsuo 与 SDK/Desktop 仍由后续工作完成，`Partial` | 管理身份参数并禁止 SDK 自设默认值 | Security & Cryptography | SM2 官方向量、OpenSSL oracle、strict DER/key/suite 负向测试、PKCS#11 目标设备互认证、跨 SDK/CLI/Desktop 测试 | `G2` |
| `CRY-04` | `STD-SM4` `STD-BLOCK-MODE`：静态秘密和备份保护 | 已固定 SM4 primitive、百万次迭代与 V2 SM4-GCM 参数；[`ADR-0010`](ADR-0010-AUTHENTICATED-SM4-SOFTWARE-KEY-ENVELOPES.zh-CN.md) 已实现 software-key envelope：随机 DEK/96-bit nonce、128-bit tag、descriptor/provider/KDF metadata AAD、严格 KDF 上下界、atomic rewrap 和负向测试。内置 passphrase KEK 仅限开发；backup v5 与生产 HSM/KMS adapter 仍待完成，`Partial` | KEK 存 HSM/KMS，实施分权、轮换、nonce 生命周期和恢复 | Security & Cryptography + Platform / SRE | envelope 规范、AAD/nonce/tag/KDF 负向测试、轮换和恢复报告 | `G3` `G5` |
| `CRY-05` | `STD-MODULE` `RULE-CII-CRYPT`：生产私钥处于受控密码模块 | 核心已使用 provider-neutral Signer、不可导出 KeyHandle、公钥描述和 capability gate；[`ADR-0008`](ADR-0008-VERSIONED-KEY-DESCRIPTORS.zh-CN.md) 已实现 canonical software/remote/PKCS#11/SDF descriptor、单 provider 解析、capability/public-key exact match、secret-safe diagnostics 和无 fallback。PKCS#11 已实现独立 native sidecar、owner-only PIN file、private/sensitive/always-sensitive/non-extractable/never-extractable/sign 属性 gate、session/token 恢复和 SoftHSM CI。SDF 已实现隔离 sidecar、稳定 deployment-owned adapter ABI、固定 SM2 digest/签名语义、owner-only 配置和凭证文件、设备/公钥漂移拒绝、fake adapter/race/跨平台门禁以及人工审批真机 workflow；目标商用密码设备仍须按具体型号、固件、SDK、OS/架构和配置完成互认证与适用检测认证，`Partial` | 采购适用且在触发时经检测认证合格的设备/服务，并执行密钥仪式 | Security & Cryptography + Certification Applicant | ADR-0004/0008、PKCS#11 contract/SoftHSM、SDF contract/fake/目标设备结果、产品证书、密钥仪式 | `G3` `G7` |
| `CRY-06` | `STD-39786`：完整密钥生命周期 | v1 descriptor 已记录 KeyID、suite、算法、provider、公钥、证书链和 provider reference；[`ADR-0009`](ADR-0009-SUITE-AWARE-KEY-REGISTRY-V2.zh-CN.md) 的 Registry V2 已实现不可变 suite manifest、canonical 签名/hash 链、SM2/INTL 注册、幂等导入、原子轮换、时点查询、撤销、compromise、证书有效期约束和崩溃尾帧恢复；[`ADR-0010`](ADR-0010-AUTHENTICATED-SM4-SOFTWARE-KEY-ENVELOPES.zh-CN.md) 已补齐软件私钥 envelope 与 KEK rewrap。PKCS#11 sidecar 已拒绝同 URI 公钥漂移，并要求以新 `CKA_ID`/descriptor/Registry 事件显式轮换；SDF sidecar 已对完整设备身份、公钥、credential reference、非零 key/KEK index 和 KEK identity fail closed，并提供不暴露明文 SM4 key 的 canonical wrapped-key envelope。硬件内生成、设备备份恢复、KEK/私钥销毁和密钥仪式仍须由目标设备受控流程验证，`Partial` | 审批生成、分发、启用、备份、恢复、销毁 | Security & Cryptography | ADR-0008/0009/0010、PKCS#11/SDF 轮换与故障测试、密钥台账、事件链、设备备份与销毁记录 | `G3` |
| `CRY-07` | `STD-CERT`：证书链和状态可验证 | descriptor 已支持受限 leaf-first DER chain、leaf 公钥精确绑定、suite/signature profile、CA/order 检查；Registry V2 将 key validity 收口到 leaf `[NotBefore, NotAfter)`，并明确 evidence chain 不是 trust root。CRL/OCSP 和签名时状态材料仍待 #455，`Partial` | 管理 CA、CRL/OCSP、双证书和有效期 | Security & Cryptography + External Crypto & BCOS Operators | ADR-0008/0009、证书链/错序/错 key/错 suite/有效期负向测试、台账 | `G3` |
...
| `EVD-03` | 证明连续性：算法和 LogID 不可混用 | file/Pebble/TiKV 非空 namespace 已固定 suite，backup/restore 与 metastore migration 已拒绝跨 suite 数据；V2/V5 仍采用破坏性切换并创建新 LogID/namespace，不迁移或重写旧证据。`Partial` | 审批停机、清空和重新初始化，确认无生产历史需要保留 | Engineering + Deployment Owner | suite marker、旧格式拒绝测试、切换记录 | `G1` `G2` |
...
| 版本 | 日期 | 变化 | 状态 |
| --- | --- | --- | --- |
| `0.1.12` | 2026-07-24 | 实现隔离 PKCS#11 sidecar、不可导出 key policy、显式机制 gate、故障分类、轮换保护与 SoftHSM 互操作 | Engineering baseline |
| `0.1.11` | 2026-07-24 | 实现 suite-aware Registry V2、SM2/INTL 密钥生命周期、原子轮换、证书有效期和崩溃恢复 | Engineering baseline |
| `0.1.10` | 2026-07-24 | 实现 canonical key descriptor、provider reference、证书链、resolver 和无 fallback 配置边界 | Engineering baseline |
| `0.1.9` | 2026-07-24 | 实现 canonical SM2-SM3 signer/verifier、固定 user ID、strict DER/公钥校验与 suite-bound 签名输入 | Engineering baseline |
| `0.1.8` | 2026-07-24 | 实现 SM3 primitive、RFC6962-SM3 Merkle Profile、suite-aware proof core 与跨 suite 拒绝 | Engineering baseline |
| `0.1.7` | 2026-07-24 | 为 file、Pebble、TiKV 建立不可变 suite marker，并绑定 backup/restore 与 metastore migration | Engineering baseline |
