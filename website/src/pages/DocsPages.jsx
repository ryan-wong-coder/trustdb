...
export function SourceBuildPage({ route }) {
  return (
    <DocsShell route={route}>
      ...
      <section className="doc-section"><h2>服务器与 CLI</h2><p>需要 Git 与 Go 1.26.2 或更高的兼容版本。固定源码标签，先测试，再生成单个 <code>trustdb</code> 二进制文件。</p><CodeBlock>git clone --branch v1.0.0-beta.1 --depth 1 https://github.com/ryan-wong-coder/trustdb.git{"\n"}cd trustdb{"\n"}go test ./...{"\n"}go build -trimpath -o ./bin/trustdb ./cmd/trustdb{"\n"}./bin/trustdb version</CodeBlock></section>
