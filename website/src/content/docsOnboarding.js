...
const zhCN = {
  ...
  quickStart: {
    ...
    platformNoteBody: "Linux 使用 sha256sum -c。Windows 请下载对应 ZIP 和 SHA256SUMS，用 Get-FileHash -Algorithm SHA256 比对后解压；后续命令在 PowerShell 中使用 .\\bin\\trustdb.exe。",
    ...
    keysBody: "客户端私钥签名原文摘要，服务端私钥签发收据。公钥用于验证。",
    keysWarning: "keygen 会替换同名密钥文件。只在新的练习目录执行一次；不要对已经签发过证据的身份重复运行。",
  ...
  server: {
    ...
    dockerBoundary: "Docker 默认把演示客户端私钥与服务端状态放在同一卷，仅用于快速体验。生产环境应在业务系统中单独持有客户端私钥，只向服务端配置相应公钥或受信任注册表。",
  ...
  troubleshooting: {
    ...
    introTitle: "先保存现场",
    introBody: "记录版本、完整错误、启动参数和相关路径权限。不要重新运行 keygen、删除 Pebble/WAL、覆盖备份或把私钥贴到公开 issue。",
    ...
    cards: [
      ...
      ["signature verification failed / key mismatch", "原文件变了，或验证者使用了错误的客户端/服务端公钥；也可能有人重新运行 keygen 覆盖了历史身份。", "核对原文件摘要与可信渠道保存的公钥指纹。保留现有文件，不要重新生成密钥来“修复”历史证据。"],
...
const en = {
  ...
  quickStart: {
    ...
    allPlatforms: "Choose another platform", platformNoteTitle: "Linux and Windows", platformNoteBody: "Linux uses sha256sum -c. On Windows, download the matching ZIP and SHA256SUMS, compare Get-FileHash -Algorithm SHA256, then extract it and use .\\bin\\trustdb.exe in PowerShell for the remaining commands.",
    ...
    keysTitle: "3. Generate one-time client and server keys", keysBody: "The client private key signs the content digest; the server private key signs receipts. Public keys are used for verification.", keysWarning: "keygen replaces same-name key files. Run it once in a new practice directory; never rerun it for an identity that has already issued evidence.", keysExpected: ".trustdb-dev contains client.key, client.pub, server.key, and server.pub. Never send private keys to a verifier.",
  ...
  server: {
    ...
    dockerBoundary: "Docker stores the demo client private key beside server state for convenience only. In production, keep client private keys in the business application and configure only the matching public key or trusted registry on the server.",
  ...
  troubleshooting: {
    ...
    introTitle: "Preserve the scene first", introBody: "Record the version, full error, startup arguments, and path permissions. Do not rerun keygen, delete Pebble/WAL, overwrite backups, or paste private keys into a public issue.", diagnosticsTitle: "Common diagnostics", causeLabel: "Likely cause", actionLabel: "Safe next step",
    cards: [
      ...
      ["signature verification failed / key mismatch", "The source changed, the verifier has the wrong client/server key, or keygen replaced an identity that already issued evidence.", "Compare the source digest and fingerprints saved through trusted channels. Preserve current files; generating new keys cannot repair historical evidence."],
